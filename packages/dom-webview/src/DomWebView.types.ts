...
export interface DomWebViewProps
  ...
  /**
   * Article selection menu. `copyComment` keeps system Copy and adds a
   * Comment action; other WebViews stay on the default edit menu.
   * @platform ios
   * @default 'default'
   */
  selectionMenu?: 'copyComment' | 'default'

  /**
   * Title of the Comment action when `selectionMenu` is `copyComment`.
   * @platform ios
   */
  selectionCommentTitle?: string

  /**
   * Title of the paragraph-comment action when `selectionMenu` is `copyComment`.
   * @platform ios
   */
  selectionBlockTitle?: string
...
interface IosScrollViewProps {
  ...
  /**
   * iOS 26+ `UIScrollEdgeEffect` on the WKWebView's scroll view.
   * Ignored on earlier iOS versions.
   * @platform ios
   */
  scrollEdgeEffects?: {
    bottom?: 'automatic' | 'hard' | 'hidden' | 'soft'
    left?: 'automatic' | 'hard' | 'hidden' | 'soft'
    right?: 'automatic' | 'hard' | 'hidden' | 'soft'
    top?: 'automatic' | 'hard' | 'hidden' | 'soft'
  }
 ...
 * are intentionally listed here even though file access is wired up natively —
 * they're enabled implicitly so DOM bundles served from `file://` work without
 * configuration. Listing them here lets the shared wrapper pass the props
 * through without TS errors.
 */
