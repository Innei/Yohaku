...
export type { BlockAnchor, RangeAnchor }
...
export { computeBlockFingerprint }
...
function getBlockTextContent(container: Element): string {
  const walker = createTextWalker(container)
  let result = ''
  let node: Node | null
  while ((node = walker.nextNode())) {
...
export function buildBlockAnchorFromSelection(
  sel: Selection,
  contentEl: Element,
  blockInfos: BlockInfo[],
  lang?: string | null,
  ...
  if (anchorBlockIdx < 0 || focusBlockIdx < 0) return null
  if (anchorBlockIdx !== focusBlockIdx) return null
  return buildBlockAnchorFromIndex(blockInfos, anchorBlockIdx, lang)
}
